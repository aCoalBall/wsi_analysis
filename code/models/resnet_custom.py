...
def resnet50_overclr():
    checkpoint = torch.load('models/checkpoints/checkpoint_0019.pth.tar')
    state_dict = checkpoint['state_dict']
    for k in list(state_dict.keys()):
        # retain only encoder_q up to before the embedding layer
        if k.startswith("module.encoder_q") and not k.startswith(
            "module.encoder_q.head"
        ):
            # remove prefix
...
def load_pretrained_weights_from_local(model, pth_file_name):
    pretrained_dict = torch.load(pth_file_name)
    model.load_state_dict(pretrained_dict, strict=False)
    return model
